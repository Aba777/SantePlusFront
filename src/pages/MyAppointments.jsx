...
const MyAppointments = () => {
  ...
  const slotDateFormat = (slotDate) => {
    const dateArray = slotDate.split("_"); // Assurez-vous que le format de la date est "JJ-MM-AAAA"

    if (dateArray.length !== 3) return "Date invalide";
  ...
  const handlePayment = async (appointmentId) => {
    try {
      ...
      if (data.success) {
        toast.success(data.message);
        getUserAppointments(); // Rafraîchir la liste des rendez-vous après paiement
      } else {
        toast.error(data.message);
      }
    } catch (error) {
